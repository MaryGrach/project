...
// Запрос в бд для авторизации - поиск по email, сравнение найденного хэша в бд с указанным паролем
...
func (repo *UserRepo) UpdateUserPassword(userID int, newPassword string) error {
	...
	// хэширование пароля (в бд не хранятся пароли, а закодированные пароли - хэши)
	...
	// запись хэша по айди пользователя
...
// Достать хэш пароля по айди пользователя
// Используется сугубо для сравнения на бэке - на фронт хэш не возвращается!
func (repo *UserRepo) GetHashPassword(userID int) (string, error) {
	var hash []string
	ctx := context.Background()

	err := pgxscan.Select(ctx, repo.db, &hash, `SELECT passwrd FROM user_data WHERE id = $1`, userID)
